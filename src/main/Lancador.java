...
public class Lancador extends javax.swing.JFrame {
    ...
    public Lancador() {
        ...
        if(Login.turnoAtual != null){
            btnAbrirTurno.setText("Fechar Turno");
        }else{
    ...
    public void fecharTurno(){
        tDao = new TurnoDAO();
        Login.turnoAtual.setStatus(0);
        String horario = Login.turnoAtual.dataAtual();
        Login.turnoAtual.setDataFechamento(horario);
        tDao.update(Login.turnoAtual);
        Login.turnoAtual = null;
        btnAbrirCaixa.setEnabled(false);
    }
    
    public void abrirTurno(){
        tDao = new TurnoDAO();
        Turno novoTurno = new Turno(1);
        novoTurno.setDataAbertura(novoTurno.dataAtual());
        tDao.create(novoTurno);
        for(Turno t: tDao.read()){
            if(t.getStatus() == novoTurno.getStatus()){
                novoTurno.setIdTurno(t.getIdTurno());
            }
        }
        Login.turnoAtual = novoTurno;
    ...
    private void initComponents() {
        ...
        btnGerenciador.setFont(new java.awt.Font("Comic Sans MS", 0, 16)); // NOI18N
    ...
    private void btnAbrirTurnoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAbrirTurnoActionPerformed
        ...
        if(Login.turnoAtual != null){
            if(Login.caixaAtual != null){
                JOptionPane.showMessageDialog(null, "Não é possível fechar o turno com o caixa aberto!");
            }else{
                fecharTurno();
                btnAbrirTurno.setText("Abrir Turno");
            }
    ...
    private void btnGerenciadorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGerenciadorActionPerformed
        if(Login.turnoAtual != null){
            ...
            JOptionPane.showMessageDialog(null, "O turno deve estar aberto!");
        }
