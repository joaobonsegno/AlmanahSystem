...
public class VendaCarteiraDAO {
    FuncionarioDAO fDao = new FuncionarioDAO();
    
    public void create(VendaCarteira v){
        Connection con = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;
        
        try{
            String sql = "INSERT INTO vendaCarteira (total, data, idCaixa, idCliente) VALUES(?,?,?,?)";
            stmt = con.prepareStatement(sql);
            stmt.setDouble(1, v.getTotal());
            stmt.setString(2, v.getData());
            stmt.setInt(3, v.getCaixa().getIdCaixa());
            stmt.setInt(4, v.getCliente().getId());
            
            stmt.executeUpdate();
        }catch(SQLException ex){
            System.err.println("Erro SQL (CREATE do VendaCarteiraDAO): "+ex);
        }finally{
            ConnectionFactory.closeConnection(con, stmt);
        }
    }
    
    public VendaCarteira readLast(){
        CaixaDAO caixaDao = new CaixaDAO();
        Connection con = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;
        ResultSet rs = null;
        VendaCarteira v = new VendaCarteira();
        
        try{
            stmt = con.prepareStatement("SELECT * FROM vendaCarteira ORDER BY idVendaCarteira DESC LIMIT 1");
            rs = stmt.executeQuery();
            while (rs.next()){
                
                
                v.setId(rs.getInt("idVendaCarteira"));
